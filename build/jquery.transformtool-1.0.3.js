/**
 * This file contains the jQuery.transformTool v1.0.3 plugin.
 * 
 ...
 * 
 * Improved for compatibility with KinectJS 5.0.1
 * by José SAYAGO http://github.com/laelitenetwork
 */
(function($) {
    /**
     * Namespace.
    ...
    function TransformToolGroup(target, options) {
        ...
        rotateGroup.setAttr('draggable',true);
            ...
            dragBoundFunc: function(pos) {
                if (this.isDragging()) {
                    ...
                    if( pos.x > p.x && pos.y > p.y ) {
                        rotateGroup.rotate( -parseFloat(angle) );
                    } else if( pos.x < p.x && pos.y < p.y ) {
                        rotateGroup.rotate( parseFloat(angle) );
                    }
                }
    ...
    TransformToolGroup.prototype.apply = function () {
        ...
        this._target.setSize({width: width, height: height});
        this._target.setOffset({x: width / 2, y: height / 2});
    };
    ...
    function hideTransformTool(target, options) {
        initTransformTool(target, options, function (tool) {
            ...
            rotateGroup.setAttr('draggable',false);
            tool.hide();
            stage.draw();
        });
    }
